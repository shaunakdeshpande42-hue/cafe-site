...
export default function Navbar() {
  ...
  // switches to opaque once the user scrolls past 20 px.
  const isOpaque = scrolled || !isHome;
  const linkColor = isOpaque ? "text-charcoal hover:text-burgundy" : "text-cream hover:text-cream/70";
  const iconColor = isOpaque ? "text-charcoal hover:text-burgundy" : "text-cream hover:text-cream/70";
  const barColor  = isOpaque ? "bg-charcoal" : "bg-cream";
          ...
          <Link href="/" className="flex items-center shrink-0">
            <Image
              ...
              className={`object-contain transition-all duration-300 ${
                isOpaque ? "" : "brightness-0 invert opacity-90"
              }`}
              priority
              unoptimized
            />
          ...
          <div className="hidden md:flex items-center gap-8">
            {navLinks.map(({ href, label }) => (
              <Link
                key={href}
                href={href}
                className={`text-sm tracking-widest uppercase font-sans transition-colors duration-200 ${linkColor}`}
              >
                {label}
              </Link>
            ))}
